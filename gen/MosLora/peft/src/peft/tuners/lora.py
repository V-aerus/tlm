...
class Linear(nn.Linear, LoraLayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, "lora_A"):
            ...
            if self.lora_use_mixer:
                # nn.init.orthogonal_(self.lora_AB.weight)
                # nn.init.xavier_uniform_(self.lora_AB.weight)
    ...
    def get_weight_B(self):
        # res = [l.weight for l in self.lora_B]
        # return self.scale_b*torch.sum(torch.stack(res, dim=-1).to(device=self.lora_B[0].weight.device), dim=-1, keepdim=False)
        ...
        if not mode and self.merge_weights and not self.merged:
            ...
            if self.r > 0:
                self.weight.data += (
                    ...
                    transpose(self.get_weight_B() @ self.get_weight_A(), self.fan_in_fan_out) * self.scaling
                )
            self.merged = True
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.disable_adapters:
            if self.r > 0 and self.merged:
                matmul_output = self.get_weight_B() @ self.get_weight_A()
            ...
            if self.r > 0:
                ...
                result += self.forward_B(self.forward_A(self.lora_dropout(x.to(self.lora_A.weight.dtype)))) * self.scaling
        else:
            result = F.linear(x, transpose(self.weight, self.fan_in_fan_out), bias=self.bias)
