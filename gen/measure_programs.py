...
def main(args):
        ...
        for line in lines:
            if line.strip():  # 只处理非空行
                try:
                    ...
                    print(f"警告：跳过无效的测量结果行: {line[:100]}... 错误: {e}")
                    continue
    ...
    for line in lines:
        if line.strip():  # 只处理非空行
            try:
                ...
                print(f"警告：跳过无效的JSON行: {line[:100]}... 错误: {e}")
                continue
    ...
    print(f"已测量记录数: {len(measured_set)}")
    print(f"待测量记录数: {len(to_measure_list)}")
