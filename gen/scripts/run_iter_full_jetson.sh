...
# Jetson (Xavier) iterative pipeline: bucket-only (NO HwKVAligner KV injection).
...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash gen/scripts/run_iter_full_jetson.sh <idx> [hw] [prev_tag] [stage] [--dry-run]"
  echo "  idx: non-negative integer (0,1,2,...)"
  echo "  hw: xavier (optional, default: EDGE_HW or xavier)"
  ...
  echo ""
  echo "Notes:"
  echo "  - This Jetson script disables KV injection. It uses --use_bucket only."
  echo "  - Bootstrap behavior: if idx==0 and no prev expert exists, it will skip LoRA gen/measure and train v1_init."
  echo "  - For manual/offloaded measurement (copy gen -> Jetson, copy measured back), set: EDGE_MANUAL_MEASURE=1"
  exit 1
fi
...
if [[ "$HW" != "xavier" ]]; then
  echo "Invalid hw: $HW (expect xavier)"
  exit 1
fi
...
if [[ "$MANUAL_MEASURE" == "1" ]]; then
  ...
  if [[ "$need" == "1" ]]; then
    ...
    echo "  1) Copy gen json(s) to Jetson:"
    echo "     - $BASE_GEN_PATH"
    if [[ "$BOOTSTRAP_NO_PREV" != "1" ]]; then
      echo "     - $LORA_GEN_PATH"
    fi
    echo "  2) Run measurement on Jetson (target: '$TARGET') and copy results back into:"
