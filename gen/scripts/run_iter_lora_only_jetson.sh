...
# Jetson (Xavier) iterative pipeline: LoRA-only stage (bucket-only, NO HwKVAligner KV injection).
...
if [[ $# -lt 1 ]]; then
  echo "Usage: bash gen/scripts/run_iter_lora_only_jetson.sh <idx> [hw] [prev_tag] [stage] [--dry-run]"
...
if [[ "$MANUAL_MEASURE" == "1" ]]; then
  ...
  if [[ "$need" == "1" ]]; then
    ...
    echo "  1) Copy gen json to Jetson:"
    echo "     - $LORA_GEN_PATH"
    echo "  2) Run measurement on Jetson (target: '$TARGET') and copy result back into:"
